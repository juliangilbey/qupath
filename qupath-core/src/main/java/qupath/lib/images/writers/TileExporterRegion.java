 ...
 *
 * This modified version of TileExporter allows exporting just a restricted
 * rectangular region of the image.
 *
 * @author Pete Bankhead
 * @author Julian Gilbey
 */
public class TileExporterRegion  {

	private final static Logger logger = LoggerFactory.getLogger(TileExporterRegion.class);
	...
	private int regionX = 0, regionY = 0;
	private int regionWidth = -1, regionHeight = -1;
	 ...
	 * @param imageData
	 */
	public TileExporterRegion(ImageData<BufferedImage> imageData) {
	 ...
	 */
	public TileExporterRegion tileSize(int tileSize) {
	 ...
	 * @return this exporter
	 */
	public TileExporterRegion tileSize(int tileWidth, int tileHeight) {
	 ...
	 * @return this exporter
	 */
	public TileExporterRegion channels(int... channels) {
	 ...
	 * @return this exporter
	 */
	public TileExporterRegion channels(String... channelNames) {
		this.server = new TransformedServerBuilder(this.server)
	 ...
	 * @return this exporter
	 */
	public TileExporterRegion overlap(int overlap) {
	 ...
	 * @return this exporter
	 */
	public TileExporterRegion overlap(int overlapX, int overlapY) {
	...
	/**
	 * Specify bounding box of region to be exported in pixels.
	 *
	 * If the width is negative, the bounding box is taken to extend
	 * to the right hand edge, and similarly for the height.
	 * @param x
	 * @param y
	 * @param width
	 * @param height
	 * @return this exporter
	 */
	public TileExporterRegion regionLimits(int x, int y, int width, int height) {
		this.regionX = x;
		this.regionY = y;
	 ...
	 */
	public TileExporterRegion downsample(double downsample) {
	 ...
	 * @return this exporter
	 */
	public TileExporterRegion requestedPixelSize(double pixelSize) {
	 ...
	 * @return this exporter
	 */
	public TileExporterRegion includePartialTiles(boolean includePartialTiles) {
	 ...
	 * @return this exporter
	 */
	public TileExporterRegion annotatedTilesOnly(boolean annotatedTilesOnly) {
	 ...
	 * @return this exporter
	 */
	public TileExporterRegion imageExtension(String ext) {
		if (!ext.startsWith("."))
			ext = "." + ext;
		this.ext = ext;
		return this;
	 ...
	 * @return this exporter
	 */
	public TileExporterRegion labeledImageExtension(String ext) {
		if (!ext.startsWith("."))
			ext = "." + ext;
		this.extLabeled = ext;
		return this;
	 ...
	 * @return this exporter
	 */
	public TileExporterRegion labeledServer(ImageServer<BufferedImage> server) {
	...
	public void writeTiles(String dirOutput) throws IOException {
		...
		var requests = getTiledRegionRequests(server,
				downsample, tileWidth, tileHeight, overlapX, overlapY,
	...
	static Collection<RegionRequest> getTiledRegionRequests(
			...
			int tileWidth, int tileHeight, int xOverlap, int yOverlap,
		...
		for (int t = 0; t < server.nTimepoints(); t++) {
			for (int z = 0; z < server.nZSlices(); z++) {
				requests.addAll(
						splitRegionRequests(fullRequest, t, z, tileWidth, tileHeight, xOverlap, yOverlap, regionX, regionY, regionWidth, regionHeight, includePartialTiles)
						);
			}
		}
		return requests;
	...
	static Collection<RegionRequest> splitRegionRequests(
		...
		int fullMinX = request.getMinX();
		int fullMinY = request.getMinY();
		int fullMaxX = request.getMaxX();
		int fullMaxY = request.getMaxY();

		int regionMinX = regionX < fullMinX ? fullMinX : regionX;
		int regionMinY = regionY < fullMinY ? fullMinY : regionY;
		int regionMaxX = regionWidth < 0 ? fullMaxX : regionX + regionWidth;
		if (regionMaxX > fullMaxX) regionMaxX = fullMaxX;
		int regionMaxY = regionHeight < 0 ? fullMaxY : regionY + regionHeight;
		if (regionMaxY > fullMaxY) regionMaxY = fullMaxY;

		int scaledMinX = (int)(fullMinX / downsample);
		int scaledMinY = (int)(fullMinY / downsample);
		int scaledMaxX = (int)(fullMaxX / downsample);
		int scaledMaxY = (int)(fullMaxY / downsample);

		int scaledRegionMinX = (int)(regionMinX / downsample);
		int scaledRegionMinY = (int)(regionMinY / downsample);
		int scaledRegionMaxX = (int)(regionMaxX / downsample);
		int scaledRegionMaxY = (int)(regionMaxY / downsample);

		for (int y = scaledRegionMinY; y < scaledRegionMaxY; y += tileHeight-yOverlap) {
			int th = tileHeight;
			if (y + th > scaledRegionMaxY)
				th = scaledRegionMaxY - y;
			...
			for (int x = scaledRegionMinX; x < scaledRegionMaxX; x += tileWidth-xOverlap) {
				int tw = tileWidth;
				if (x + tw > scaledRegionMaxX)
					tw = scaledRegionMaxX - x;
